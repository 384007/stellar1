# Shot Lab — 唯一提示词（全量规格 + 工程任务）

**仓库**：`stellar-ai` · **主应用**：`frontend/`（Next.js / Edge / Cloudflare，复用 `frontend/lib/*`、`/api/analyze` 等现有能力）。

**用法**：在 Cursor 对话里只 `@` 本文件 **`docs/shot-lab-opus-prompt.zh.md`** 即可；**不要**再 `@` `shot-lab-master-prompt.zh.md` 或 `opus-shot-lab-full-task.zh.md`（已并入本文）。

**执行铁律**：从「角色指令」读到文末全部落地；禁止只做 MVP、禁止反问是否分阶段；禁止假数据与仅前端冒充权限；禁止改 Secrets、禁止 `deploy` / `preview` / `upload`；客观做不到只写入 **`docs/shot-lab-backlog.md`**（须写清原因与后续条件）。允许多 PR，但每 PR 须 `cd frontend && npm install && npm run build` 通过。开工先输出 **(1) 差距分析 (2) 分 PR 计划**，再从 D1、`lab-db`、`/api/lab` 权限与裁剪做起。

**可选**：英文术语 `docs/shot-lab-master-prompt.en.md`。
...
（英文版见仓库 `docs/shot-lab-master-prompt.en.md`。）
...
以下为仓库内必读路径、硬约束与验收顺序；产品逻辑以上文第一～十四节为准。规格中的「第一阶段 / 第二阶段」仅为文档分法——**能实现的尽量在本仓一次做完**；客观阻塞写入 `docs/shot-lab-backlog.md`。
...
| 路径 | 说明 |
|------|------|
| `docs/shot-lab-master-prompt.en.md` | 英文术语与文案 |
| `README.md` | 整体架构、D1/R2、部署栈 |

### 2.2 数据层（D1）

| 路径 | 说明 |
|------|------|
| `schema/0004_shot_lab.sql` | 现有：`lab_jobs`、`lab_usage_daily`、`lab_quota_log` |
| `schema/d1_schema.sql` | 总览 |
| 新迁移 | 在 `schema/` 新增 **下一个未占用编号** 的 `.sql`（当前仓库若已有 `0006`，则新建 `0007_*.sql` 等），**禁止**破坏已有表语义 |

### 2.3 前端 Shot Lab 与 API（必须先读再改）

| 路径 | 说明 |
|------|------|
| `frontend/app/shot-lab/page.tsx` | Shot Lab 主页面 |
| `frontend/app/api/lab/route.ts` | `POST /api/lab` |
| `frontend/app/api/lab/history/route.ts` | Lab 历史列表 |
| `frontend/app/api/lab/[id]/route.ts` | 单条 Lab 记录 |
| `frontend/lib/lab-db.ts` | D1 访问 |
| `frontend/lib/lab-config.ts` | 开关、限额、裁剪常量 |
| `frontend/lib/upload-video.ts` | 两阶段上传 |
| `frontend/lib/fetch-retry.ts` | 重试 |
| `frontend/lib/strip-result.ts` | 存储裁剪 |

### 2.4 入口与导航（最小改动）

| 路径 | 说明 |
|------|------|
| `frontend/app/page.tsx` | 首页 Shot Lab CTA |
| `frontend/components/HeroSection.tsx` | Hero Shot Lab 按钮 |
| `frontend/app/history/page.tsx` | 历史与 lab 联动 |

### 2.5 Python（本仓库）

| 目录 | 说明 |
|------|------|
| `backend/services/` | 与上部**第八节**算法一致时修改或新增 |

### 2.6 组件复用边界

| 路径 | 与 Shot Lab 关系 |
|------|------------------|
| `frontend/components/VideoAnalysisOverlay.tsx` | 勿强行把 Shot Lab 塞进巨型组件；可抽 `components/lab/` 小模块 |
| `frontend/components/PlusResultView.tsx` | Plus 专用类型；与 Lab **字段/类型不混用** |
| `frontend/components/ScreenModeCapture.tsx` | 已复用；改动需回归 `/analyze`、`/plus`、`/shot-lab` |

---

### 仓库级硬约束（违反即失败）

1. 仅本仓库；**不**改 GitHub Secrets、Cloudflare 变量/绑定；**不**执行 `wrangler deploy`、`npm run deploy` / `preview` / `upload`。  
2. 持久化：**D1 + R2**；**禁止**第二套 DB 栈。  
3. **不得破坏** `/analyze`、`/plus`、`/pro`、`/history`；Shot Lab 路由 **`/shot-lab`** 与全仓一致。  
4. 新能力优先 **`/api/lab`**。  
5. Free/Pro **服务端强制**；前端非唯一防线。  
6. estimate + confidence；禁止伪装雷达精度。
...
- **第四节** 功能分层：逐项 UI/接口落地。  
- **第五节** 权限计费：`lab_quota_log` 幂等、历史服务端裁剪。  
- **第九节** API：在现有 `POST/GET /api/lab` 上扩展 compare、trend、export（Pro）；错误码统一。  
- **第三节** UI：含 **屏幕/对屏** 提示与骨架降级（真实用户问题）。  
- **第八节** 关键帧：**同一视频时间轴**；禁止参考图混入；`LAB_PROMPT` 可见性约束。

---

### 实施顺序

1. **差距分析**（对话输出）：上部规格 vs `shot-lab` + `api/lab` + `lab-db` + `0004_shot_lab.sql`。  
2. **分 PR 计划**。  
3. D1 migration → `lab-db` / `lab-config` → API → `LAB_PROMPT` + schema → UI → Pro 功能 → `docs/shot-lab-backlog.md`。  
4. **自检**：上部 **第十三节** 清单写入 PR 描述。
...
1. 源码与 SQL 路径列表。  
2. `docs/shot-lab-backlog.md`。  
3. PR 描述：可见变化、API、迁移、验收路径。  
4. 勿提交无关构建垃圾。
...
请先输出 **(1) 差距分析** **(2) 分 PR 计划**。  
**不要**反问是否做 MVP——默认 **全量按上部规格在本仓可实现范围内落地**；阻塞项只写入 `docs/shot-lab-backlog.md`。  
然后从 **D1 + lab-db + API 权限** 开始第一批补丁。


---

**阻塞登记**：`docs/shot-lab-backlog.md`
